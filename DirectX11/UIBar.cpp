...
void UIBar::SetScale(const DirectX::XMFLOAT2& scale, const DirectX::XMFLOAT2& offset)
{
	m_scale = { scale.x,scale.y,1.0f };
	// Set Scale
	m_barMesh->GetTransform().SetScale(m_scale);
	// 背景ちょっと大きい
	DirectX::XMFLOAT3 bgScale = { m_scale.x + offset.x,m_scale.y + offset.y ,m_scale.z };
	m_backgroundMesh->GetTransform().SetScale(bgScale);

}

void UIBar::SetPosition(const DirectX::XMFLOAT3& pos, const DirectX::XMFLOAT2& offset)
{
	m_position = pos;
